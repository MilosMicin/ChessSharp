...
namespace ChessSharp.ChessLogicF {
    ...
    public partial class frmLoadPGNGames : Window {
        ...
        private ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE   m_ePhase;
        ...
        private delegate void   delProgressCallBack(ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE ePhase, int iFileIndex, int iFileCount, string strFileName, int iGameDone, int iGameCount);
        ...
        private void WndCallBack(ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE ePhase, int iFileIndex, int iFileCount, string strFileName, int iGameDone, int iGameCount) {
            if (m_ePhase != ePhase) {
                switch (ePhase) {
                case ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE.OpeningFile:
                    ctlPhase.Content                = "Openning the file";
                    ctlFileBeingProcessed.Content   = System.IO.Path.GetFileName(strFileName);
                    ctlStep.Content                 = "";
                    break;
                case ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE.ReadingFile:
                    ctlPhase.Content                = "Reading the file content into memory";
                    ctlStep.Content                 = "";
                    break;
                case ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE.RawParsing:
                    ctlPhase.Content                = "Parsing the PGN";
                    ctlStep.Content                 = "0 / " + iGameCount.ToString() + "mb";
                    break;
                case ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE.Finished:
        ...
        /// <param name="iGameCount">       Game count</param>
        static void ProgressCallBack(object cookie, ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE ePhase, int iFileIndex, int iFileCount, string strFileName, int iGameProcessed, int iGameCount) {
            frmLoadPGNGames     frm;
        ...
        private bool LoadPGN() {
            ...
            try {
                ...
                m_iTotalTruncated   = 0;
                m_strError          = null;
                m_ePhase            = ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE.None;
            ...
            m_bResult  = bRetVal;
            ProgressCallBack(this, ChessSharp.ChessLogicF.PgnParser.ParsingPhaseE.Finished, 0, 0, null, 0, 0);
            return(bRetVal);
        }
